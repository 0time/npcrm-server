...
d(me, () => {
  ...
  describe(GET, () => {
    describe('given a list of fields is not provided', () => {
      ...
      it('should respond with OK and the result of the query', () =>
        getASupertest().then((supertest) =>
          supertest.get('/api/donor').expect(OK, { data }),
        ));
    });
    ...
    describe('given a list of fields is provided', () => {
      ...
      beforeEach(() => {
        ...
        argSets = [
          [
            `SELECT "${field1}" , "${field2}" FROM "Donor" LIMIT '10'`,
            {
              rows: data,
            },
          ],
  ...
  describe(DELETE, () => {
    ...
    it('should respond with OK and the empty data rows result', () =>
      getASupertest().then((supertest) =>
        supertest.delete('/api/donor').send({ id }).expect(OK, { data }),
      ));
  });
  ...
  describe(PUT, () => {
    ...
    describe('create', () => {
      beforeEach(() => {
        ...
        argSets = [
          [
            `INSERT INTO "Donor" ( name ) VALUES ( $1 );`,
            [name],
            { rows: data },
          ],
        ];
      ...
      it('should respond with OK and the empty data rows result', () =>
        getASupertest().then((supertest) =>
          supertest.put('/api/donor').send({ name }).expect(OK, { data }),
        ));
    });
    ...
    describe('update', () => {
      ...
      beforeEach(() => {
        ...
        argSets = [
          [
            `UPDATE "Donor" SET name = $1 WHERE id = $2;`,
            [name, id],
            { rows: data },
          ],
        ];
      ...
      it('should respond with OK and the empty data rows result', () =>
        getASupertest().then((supertest) =>
          supertest.put('/api/donor').send({ id, name }).expect(OK, { data }),
        ));
    });
  });
});
