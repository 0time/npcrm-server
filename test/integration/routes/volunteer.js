...
d(me, () => {
  ...
  describe(GET, () => {
    describe('given a list of fields is not provided', () => {
      ...
      it('should respond with OK and the result of the query', () =>
        getASupertest().then((supertest) =>
          supertest.get('/api/volunteer').expect(OK, { data }),
        ));
    });
    ...
    describe('given a list of fields is provided', () => {
      ...
      beforeEach(() => {
        ...
        argSets = [
          [
            `SELECT "${field1}" , "${field2}" FROM "Volunteer" LIMIT '10'`,
            {
              rows: data,
            },
          ],
  ...
  describe(DELETE, () => {
    ...
    it('should respond with OK and the empty data rows result', () =>
      getASupertest().then((supertest) =>
        supertest.delete('/api/volunteer').send({ id }).expect(OK, { data }),
      ));
  });
  ...
  describe(PUT, () => {
    ...
    describe('create', () => {
      beforeEach(() => {
        ...
        argSets = [
          [
            `INSERT INTO "Volunteer" ( name ) VALUES ( $1 );`,
            [name],
            { rows: data },
          ],
        ];
      ...
      it('should respond with OK and the empty data rows result', () =>
        getASupertest().then((supertest) =>
          supertest.put('/api/volunteer').send({ name }).expect(OK, { data }),
        ));
    });
    ...
    describe('update', () => {
      ...
      beforeEach(() => {
        ...
        argSets = [
          [
            `UPDATE "Volunteer" SET name = $1 WHERE id = $2;`,
            [name, id],
            { rows: data },
          ],
        ];
