...
d(me, () => {
  ...
  describe(GET, () => {
    describe('given a list of fields is not provided', () => {
      ...
      it('should respond with OK and the result of the query', () =>
        getASupertest().then((supertest) =>
          supertest.get('/api/client').expect(OK, { data }),
        ));
    });
    ...
    describe('given a list of fields is provided', () => {
      ...
      beforeEach(() => {
        ...
        argSets = [
          [
            `SELECT "${field1}" , "${field2}" FROM "Client" LIMIT '10'`,
            {
              rows: data,
            },
          ],
  ...
  describe(DELETE, () => {
    ...
    it('should respond with OK and the empty data rows result', () =>
      getASupertest().then((supertest) =>
        supertest.delete('/api/client').send({ id }).expect(OK, { data }),
      ));
  });
  ...
  describe(PUT, () => {
    ...
    describe('create', () => {
      ...
      it('should respond with OK and the empty data rows result', () =>
        getASupertest().then((supertest) =>
          supertest.put('/api/client').send({ name }).expect(OK, { data }),
        ));
    });
    ...
    describe('update', () => {
      ...
      it('should respond with OK and the empty data rows result', () =>
        getASupertest().then((supertest) =>
          supertest.put('/api/client').send({ id, name }).expect(OK, { data }),
        ));
    });
  });
});
